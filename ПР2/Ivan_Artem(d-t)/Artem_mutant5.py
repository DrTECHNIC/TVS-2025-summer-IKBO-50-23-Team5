...
def add_task(task_list, description):
    if ",  " in description:
        ...
        print("описание задачи не может быть пустым!") # может - должно
...
def mark_task_done(task_list, index):
    try:
        ...
        print("такого индекса нет! В данный момент индексы находятся в диапазоне от 0 до " + str(len(task_list)-1) + " включительно") # нет - не существует
    except:
        print("неправильный формат ввода. Индекс должен быть неотрицательным целым числом") # неотрицательным целым - целым неотрицательным
